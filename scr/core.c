...
static void FillRichEditFromRawData(TabInfo* tab, int encoding) {
    if (!tab->rawData || tab->rawSize == 0) { SetWindowText(tab->hRichEdit, L""); return; }
    if (!tab->hexMode) {
        int bomOff = 0;
        ...
        if (encoding == 1251 || encoding == 1252) {
            ...
            if (wlen > 0) {
                WCHAR* wbuf = malloc((wlen + 1) * sizeof(WCHAR));
                if (wbuf) {
                    MultiByteToWideChar(encoding, 0, (LPCCH)(tab->rawData + bomOff), tab->rawSize - bomOff, wbuf, wlen + 1);
                    wbuf[wlen] = L'\0'; SetWindowText(tab->hRichEdit, wbuf); free(wbuf);
                }
            } else SetWindowText(tab->hRichEdit, L"");
            ...
            if (wlen > 0) {
                WCHAR* wbuf = malloc((wlen + 1) * sizeof(WCHAR));
                if (wbuf) {
                    MultiByteToWideChar(CP_UTF8, 0, (LPCCH)(tab->rawData + bomOff), tab->rawSize - bomOff, wbuf, wlen + 1);
                    wbuf[wlen] = L'\0'; SetWindowText(tab->hRichEdit, wbuf); free(wbuf);
                }
            } else SetWindowText(tab->hRichEdit, L"");
        }
        ...
        if (!buf) { SetWindowText(tab->hRichEdit, GetStr(STR_HEX_MEMORY_ERROR)); return; }
        WCHAR* ptr = buf;
        ...
        *ptr = L'\0'; SetWindowText(tab->hRichEdit, buf); free(buf);
...
void AddTab(AppState* state, const WCHAR* title, const WCHAR* filepath) {
    ...
    wcsncpy(state->tabs[idx].title, title, 255);
    if (filepath) wcsncpy(state->tabs[idx].filepath, filepath, MAX_PATH - 1);
...
void UpdateTabTitle(AppState* state, int index) {
    WCHAR title[270];
    const WCHAR* fname = state->tabs[index].title;
    if (state->tabs[index].modified) _snwprintf(title, 269, L"*%s", fname);
    else wcsncpy(title, fname, 269);
...
void UpdateWindowTitle(const AppState* state) {
    ...
    if (tab->filepath[0]) {
        ...
        wcsncpy(fname, p ? p + 1 : tab->filepath, 255);
    } else wcsncpy(fname, tab->title, 255);
    _snwprintf(caption, 511, L"%s%s - %s", tab->modified ? L"*" : L"", fname, GetStr(STR_TITLE));
...
void OnCloseTab(AppState* state) {
    ...
    if (state->tabs[idx].modified) {
        WCHAR msg[512]; _snwprintf(msg, 511, GetStr(STR_CLOSE_SAVE_PROMPT), state->tabs[idx].title);
...
BOOL LoadFileToTab(AppState* state, int index, const WCHAR* filepath, int encoding) {
    TabInfo* tab = &state->tabs[index];
    free(tab->rawData); tab->rawData = NULL; tab->rawSize = 0;
    HANDLE hFile = CreateFile(filepath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (hFile == INVALID_HANDLE_VALUE) { MessageBox(state->hwndMain, GetStr(STR_ERROR_OPEN_FILE), GetStr(STR_TITLE), MB_ICONERROR); return FALSE; }
    DWORD fileSize = GetFileSize(hFile, NULL);
    if (fileSize > MAX_FILE_SIZE) { CloseHandle(hFile); MessageBox(state->hwndMain, GetStr(STR_FILE_TOO_BIG), GetStr(STR_TITLE), MB_ICONWARNING); return FALSE; }
    if (fileSize == 0) {
        CloseHandle(hFile); SetWindowText(tab->hRichEdit, L"");
        wcscpy(tab->filepath, filepath);
        const WCHAR* p = wcsrchr(filepath, L'\\'); wcsncpy(tab->title, p ? p + 1 : filepath, 255);
        tab->modified = FALSE; tab->hexMode = FALSE; SendMessage(tab->hRichEdit, EM_SETREADONLY, FALSE, 0);
        UpdateTabTitle(state, index); RecalcGutterWidth(state, index); ResizeControls(state); return TRUE;
    }
    tab->rawData = malloc(fileSize);
    if (!tab->rawData) { CloseHandle(hFile); MessageBox(state->hwndMain, GetStr(STR_NOT_ENOUGH_MEMORY), GetStr(STR_TITLE), MB_ICONERROR); return FALSE; }
    DWORD read;
    if (!ReadFile(hFile, tab->rawData, fileSize, &read, NULL) || read != fileSize) {
        free(tab->rawData); tab->rawData = NULL; CloseHandle(hFile);
        MessageBox(state->hwndMain, GetStr(STR_ERROR_READ_FILE), GetStr(STR_TITLE), MB_ICONERROR); return FALSE;
    }
    CloseHandle(hFile); tab->rawSize = fileSize;
    ...
    if (!isText) {
        ...
        tab->hexMode = FALSE; SendMessage(tab->hRichEdit, EM_SETREADONLY, FALSE, 0);
        FillRichEditFromRawData(tab, encoding);
    }
    wcscpy(tab->filepath, filepath);
    const WCHAR* p = wcsrchr(filepath, L'\\'); wcsncpy(tab->title, p ? p + 1 : filepath, 255);
    tab->modified = FALSE; UpdateTabTitle(state, index); RecalcGutterWidth(state, index); ResizeControls(state);
    return TRUE;
}
...
BOOL SaveTab(AppState* state, int index, BOOL forceSaveAs) {
    ...
    if (!forceSaveAs && tab->filepath[0]) wcscpy(path, tab->filepath);
    else {
        WCHAR saveFilter[] = L"Text files (*.txt)\0*.txt\0All files (*.*)\0*.*\0";
        OPENFILENAME ofn = {0}; ofn.lStructSize = sizeof(ofn); ofn.hwndOwner = state->hwndMain;
        ofn.lpstrFilter = saveFilter; ofn.lpstrFile = path; ofn.nMaxFile = MAX_PATH; ofn.nFilterIndex = 2; ofn.Flags = OFN_OVERWRITEPROMPT;
        if (!GetSaveFileName(&ofn)) return FALSE;
        wcscpy(tab->filepath, path); const WCHAR* p = wcsrchr(path, L'\\'); wcsncpy(tab->title, p ? p + 1 : path, 255);
    }
    GETTEXTEX gtx = {0}; gtx.cb = GetWindowTextLength(tab->hRichEdit);
    if (gtx.cb == 0) {
        HANDLE hFile = CreateFile(path, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
        if (hFile != INVALID_HANDLE_VALUE) CloseHandle(hFile);
        tab->modified = FALSE; UpdateTabTitle(state, index); return TRUE;
    }
    gtx.cb++; WCHAR* wbuf = malloc(gtx.cb * sizeof(WCHAR)); if (!wbuf) return FALSE;
    gtx.flags = GT_DEFAULT; gtx.codepage = 1200;
    ...
    char* utf8buf = malloc(utf8len); if (!utf8buf) { free(wbuf); return FALSE; }
    WideCharToMultiByte(CP_UTF8, 0, wbuf, -1, utf8buf, utf8len, NULL, NULL); free(wbuf);
    HANDLE hFile = CreateFile(path, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (hFile == INVALID_HANDLE_VALUE) { free(utf8buf); return FALSE; }
    DWORD written; WriteFile(hFile, utf8buf, utf8len - 1, &written, NULL); CloseHandle(hFile); free(utf8buf);
    tab->modified = FALSE; UpdateTabTitle(state, index); return TRUE;
}
...
void UpdateStatusBar(const AppState* state, int index) {
    ...
    _snwprintf(lineColPart, 127, GetStr(STR_STATUS_LINE_COL), line, col);
    _snwprintf(sizePart, 255, GetStr(STR_STATUS_ENC_SIZE), enc, fileSize);
    _snwprintf(text, 511, L"  %s  |  %s", lineColPart, sizePart);
    ...
    if (uIdSubclass == SUBCLASS_GUTTER && uMsg == WM_PAINT) {
        ...
        if (totalLines > 0) {
            ...
            for (int i = 0; i < visibleLines && (firstLine + i) < totalLines; i++) {
                int lineNum = firstLine + i + 1; WCHAR buf[20]; _snwprintf(buf, 20, L"%d", lineNum);
