...
void AllowDarkModeForWindow(HWND hWnd, BOOL allow) {
    ...
    if (hUxtheme) {
        AllowDarkModeForWindow_t fn = (AllowDarkModeForWindow_t)GetProcAddress(hUxtheme, MAKEINTRESOURCEA(135));
        if (fn) fn(hWnd, allow);
...
void ApplyTheme(AppState* state) {
    ...
    state->lineNumberTextColor = lnFg;
    state->separatorColor = sepColor;
    state->statusTextColor = statusFg;
    state->statusBkColor = statusBg;
