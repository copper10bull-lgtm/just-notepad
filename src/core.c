...
static BOOL IsTextFile(const BYTE* data, DWORD size) {
    ...
    int wlen = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, (LPCCH)data, (int)size, NULL, 0);
...
static void FillRichEditFromRawData(TabInfo* tab, int encoding) {
    ...
    if (!tab->hexMode) {
        ...
        DWORD srcLen = tab->rawSize - (DWORD)bomOff;
        if (encoding == 1251 || encoding == 1252) {
            int wlen = MultiByteToWideChar((UINT)encoding, 0, (LPCCH)(tab->rawData + bomOff), (int)srcLen, NULL, 0);
            if (wlen <= 0) {
                SetWindowText(tab->hRichEdit, L"");
                return;
            }
            ...
            int conv = MultiByteToWideChar((UINT)encoding, 0, (LPCCH)(tab->rawData + bomOff), (int)srcLen, wbuf, wlen);
            if (conv > 0) {
                wbuf[conv] = L'\0';
            ...
            memcpy(wbuf, tab->rawData + bomOff, (size_t)wlen * sizeof(WCHAR));
            wbuf[wlen] = L'\0';
            SetWindowText(tab->hRichEdit, wbuf);
            free(wbuf);
            ...
            for (DWORD i = 0; i < wlen; i++) wbuf[i] = (WCHAR)((src[i*2] << 8) | src[i*2 + 1]);
            wbuf[wlen] = L'\0';
            SetWindowText(tab->hRichEdit, wbuf);
            free(wbuf);
        } else {
            int wlen = MultiByteToWideChar(CP_UTF8, 0, (LPCCH)(tab->rawData + bomOff), (int)srcLen, NULL, 0);
            if (wlen <= 0) {
                SetWindowText(tab->hRichEdit, L"");
                return;
            }
            ...
            int conv = MultiByteToWideChar(CP_UTF8, 0, (LPCCH)(tab->rawData + bomOff), (int)srcLen, wbuf, wlen);
            if (conv > 0) {
                wbuf[conv] = L'\0';
                SetWindowText(tab->hRichEdit, wbuf);
            } else {
        ...
        DWORD maxHexDump = 100 * 1024 * 1024;
        DWORD dumpSize = min(tab->rawSize, maxHexDump);
        DWORD lines = (dumpSize + 15) / 16;
        DWORD bufSize = lines * 80 + 1;
        WCHAR* buf = malloc((size_t)bufSize * sizeof(WCHAR));
        if (!buf) {
        ...
        for (DWORD i = 0; i < dumpSize; i += 16) {
            ...
            *ptr++ = L' '; *ptr++ = L'|';
            for (DWORD j = 0; j < 16 && (i + j) < dumpSize; j++) {
        ...
        *ptr = L'\0';
        ...
        if (dumpSize < tab->rawSize) {
            SendMessage(tab->hRichEdit, EM_SETSEL, (WPARAM)-1, (LPARAM)-1);
...
void AddTab(AppState* state, const WCHAR* title, const WCHAR* filepath) {
    if (state->tabCount >= state->tabCapacity) {
        ...
        state->tabs = realloc(state->tabs, (size_t)state->tabCapacity * sizeof(TabInfo));
    }
    int idx = state->tabCount++;
    memset(&state->tabs[idx], 0, sizeof(TabInfo));
    StringCchCopy(state->tabs[idx].title, _countof(state->tabs[idx].title), title);
    if (filepath) StringCchCopy(state->tabs[idx].filepath, _countof(state->tabs[idx].filepath), filepath);
    ...
    if (!hRichEdit) {
        ...
        if (!hRichEdit) {
            MessageBox(state->hwndMain, GetStr(STR_MSFTEDIT_NOTFOUND), L"Fatal Error", MB_ICONERROR);
            exit(1);
        }
    }
    ...
    if (!state->tabs[idx].hRichEdit) {
        MessageBox(state->hwndMain, GetStr(STR_CREATE_RICHEDIT_FAIL), L"Fatal Error", MB_ICONERROR);
        exit(1);
    }
    ...
    // Используем локальный буфер, чтобы избежать приведения const
    WCHAR tabTitle[256];
    StringCchCopy(tabTitle, _countof(tabTitle), title);
    TCITEM tie = {0};
    tie.mask = TCIF_TEXT;
...
void RemoveTab(AppState* state, int index) {
    ...
    DestroyWindow(tab->hRichEdit);
    DestroyWindow(tab->hSeparator);
    DestroyWindow(tab->hLineNumbers);
    DestroyWindow(tab->hContainer);
    free(tab->rawData);
    tab->rawData = NULL;
    TabCtrl_DeleteItem(state->hTab, index);
    if (index < state->tabCount - 1)
        memmove(&state->tabs[index], &state->tabs[index + 1], (size_t)(state->tabCount - index - 1) * sizeof(TabInfo));
    state->tabCount--;
    if (state->tabCount == 0) {
...
void UpdateTabTitle(AppState* state, int index) {
    ...
    if (state->tabs[index].modified)
        StringCchPrintf(title, _countof(title), L"*%s", state->tabs[index].title);
    else
        StringCchCopy(title, _countof(title), state->tabs[index].title);
    TCITEM tie = {0};
    tie.mask = TCIF_TEXT;
    tie.pszText = title;
...
void UpdateWindowTitle(const AppState* state) {
    ...
    if (tab->filepath[0]) {
        ...
        if (p) StringCchCopy(fname, _countof(fname), p + 1);
        else StringCchCopy(fname, _countof(fname), tab->filepath);
    } else {
        StringCchCopy(fname, _countof(fname), tab->title);
    }
    StringCchPrintf(caption, _countof(caption), L"%s%s - %s", tab->modified ? L"*" : L"", fname, GetStr(STR_TITLE));
...
void OnCloseTab(AppState* state) {
    ...
    if (state->tabs[idx].modified) {
        WCHAR msg[512];
        StringCchPrintf(msg, _countof(msg), GetStr(STR_CLOSE_SAVE_PROMPT), state->tabs[idx].title);
        int ret = MessageBox(state->hwndMain, msg, GetStr(STR_MENU_CLOSETAB), MB_YESNOCANCEL | MB_ICONQUESTION);
        if (ret == IDCANCEL) return;
        if (ret == IDYES) {
            if (!SaveTab(state, idx, FALSE)) return;
        }
    }
    RemoveTab(state, idx);
}
...
BOOL SwitchHexMode(AppState* state, int index) {
    ...
    if (tab->hexMode) {
        ...
        FillRichEditFromRawData(tab, state->defaultEncoding);
    } else {
        if (tab->modified && tab->rawData) {
            ...
            if (ret == IDYES) {
                if (!SaveTab(state, index, FALSE)) return FALSE;
            } else if (ret == IDNO) {
                tab->modified = FALSE;
            } else {
                return FALSE;
...
BOOL LoadFileToTab(AppState* state, int index, const WCHAR* filepath, int encoding) {
    ...
    if (fileSize == 0) {
        CloseHandle(hFile);
        SetWindowText(tab->hRichEdit, L"");
        StringCchCopy(tab->filepath, _countof(tab->filepath), filepath);
        const WCHAR* p = wcsrchr(filepath, L'\\');
        if (p) StringCchCopy(tab->title, _countof(tab->title), p + 1);
        else StringCchCopy(tab->title, _countof(tab->title), filepath);
        tab->modified = FALSE;
        tab->hexMode = FALSE;
        SendMessage(tab->hRichEdit, EM_SETREADONLY, FALSE, 0);
        UpdateTabTitle(state, index);
...
BOOL SaveTab(AppState* state, int index, BOOL forceSaveAs) {
    ...
    if (!forceSaveAs && tab->filepath[0]) {
        StringCchCopy(path, _countof(path), tab->filepath);
    } else {
        ...
        StringCchCopy(tab->filepath, _countof(tab->filepath), path);
        const WCHAR* p = wcsrchr(path, L'\\');
        if (p) StringCchCopy(tab->title, _countof(tab->title), p + 1);
        else StringCchCopy(tab->title, _countof(tab->title), path);
    }
    GETTEXTEX gtx = {0};
    gtx.cb = (DWORD)GetWindowTextLength(tab->hRichEdit);
    if (gtx.cb == 0) {
    ...
    DWORD written = 0;
    DWORD bytesToWrite = (DWORD)(utf8len - 1);
    BOOL ok = WriteFile(hFile, utf8buf, bytesToWrite, &written, NULL);
    CloseHandle(hFile);
    free(utf8buf);
    if (!ok || written != bytesToWrite) return FALSE;
    tab->modified = FALSE;
    UpdateTabTitle(state, index);
    return TRUE;
}
...
void UpdateStatusBar(const AppState* state, int index) {
    ...
    WCHAR text[512];
    LRESULT lineRes = SendMessage(tab->hRichEdit, EM_LINEFROMCHAR, (WPARAM)-1, 0);
    int line = (int)lineRes + 1;
    LRESULT sel = SendMessage(tab->hRichEdit, EM_GETSEL, 0, 0);
    int start = LOWORD(sel);
    LRESULT lineIndexRes = SendMessage(tab->hRichEdit, EM_LINEINDEX, (WPARAM)(line - 1), 0);
    int lineIndex = (int)lineIndexRes;
    ...
    StringCchPrintf(lineColPart, _countof(lineColPart), GetStr(STR_STATUS_LINE_COL), line, col);
    StringCchPrintf(sizePart, _countof(sizePart), GetStr(STR_STATUS_ENC_SIZE), enc, fileSize);
    StringCchPrintf(text, _countof(text), L"  %s  |  %s", lineColPart, sizePart);
    SetWindowText(state->hStatus, text);
}

void ShowFileHash(HWND hwnd, const BYTE* data, DWORD size, ALG_ID algId) {
    if (!data || size == 0) {
        MessageBox(hwnd, GetStr(STR_HASH_EMPTY), GetStr(STR_HASH_TITLE), MB_OK | MB_ICONINFORMATION);
        return;
    }
    HCRYPTPROV hProv = 0;
    HCRYPTHASH hHash = 0;
    ...
    DWORD hexLen = hashLen * 2 + 1;
    WCHAR* szHash = malloc((size_t)hexLen * sizeof(WCHAR));
    if (szHash) {
        WCHAR* ptr = szHash;
        for (DWORD i = 0; i < hashLen; i++) {
            ptr += _snwprintf(ptr, 3, L"%02X", hash[i]);
        }
        *ptr = L'\0';
        MessageBox(hwnd, szHash, GetStr(STR_HASH_TITLE), MB_OK | MB_ICONINFORMATION);
        free(szHash);
    }
    free(hash);
}
...
void ShowFindDialog(AppState* state) {
    ...
    state->fr.wFindWhatLen = _countof(state->findWhat);
...
void DoFind(HWND hwnd, FINDREPLACE* fr) {
    ...
    FINDTEXTEX ft = {0};
    LRESULT sel = SendMessage(hEdit, EM_GETSEL, 0, 0);
    if (fr->Flags & FR_DOWN) {
        ft.chrg.cpMin = (LONG)(LOWORD(sel)) + 1;
        ft.chrg.cpMax = -1;
    } else {
        ft.chrg.cpMin = 0;
        ft.chrg.cpMax = (LONG)(LOWORD(sel));
    }
    ft.lpstrText = fr->lpstrFindWhat;
    LRESULT pos = SendMessage(hEdit, EM_FINDTEXTEX, (WPARAM)findFlags, (LPARAM)&ft);
    if (pos == -1) {
        if (fr->Flags & FR_DOWN) {
            ft.chrg.cpMin = 0;
            ft.chrg.cpMax = -1;
        ...
        pos = SendMessage(hEdit, EM_FINDTEXTEX, (WPARAM)findFlags, (LPARAM)&ft);
    }
    if (pos != -1) {
        SendMessage(hEdit, EM_SETSEL, (WPARAM)pos, (LPARAM)(pos + (LRESULT)wcslen(fr->lpstrFindWhat)));
...
void RecalcGutterWidth(AppState* state, int index) {
    TabInfo* tab = &state->tabs[index];
    int totalLines = (int)SendMessage(tab->hRichEdit, EM_GETLINECOUNT, 0, 0);
    int digits = 1;
    int temp = totalLines;
        ...
        if (tab) {
            ...
            if (state) {
                ...
                for (int i = 0; i < state->tabCount; i++) {
                    if (&state->tabs[i] == tab) { idx = i; break; }
                }
                switch (uMsg) {
                    case WM_COMMAND:
...
LRESULT CALLBACK GutterSubclassProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData) {
    if (uIdSubclass == SUBCLASS_GUTTER && uMsg == WM_PAINT) {
        ...
        if (totalLines > 0) {
            ...
            int charIndex = (int)SendMessage(hEdit, EM_LINEINDEX, (WPARAM)firstLine, 0);
            POINT pt = {0};
            if (charIndex >= 0) SendMessage(hEdit, EM_POSFROMCHAR, (WPARAM)&pt, (LPARAM)charIndex);
            int topOffset = pt.y;
            int lineHeight = (int)SendMessage(hEdit, EM_GETLINEHEIGHT, 0, 0);
            if (lineHeight <= 0) {
                TEXTMETRIC tm;
                HFONT hFont = state->hFont;
                HFONT oldFont = hFont ? SelectObject(hdc, hFont) : NULL;
            ...
            HFONT hFont = state->hFont;
            HFONT oldFont = hFont ? SelectObject(hdc, hFont) : NULL;
            SetTextColor(hdc, state->lineNumberTextColor);
            ...
            for (int i = 0; i < visibleLines && (firstLine + i) < totalLines; i++) {
                ...
                WCHAR buf[20];
                StringCchPrintf(buf, _countof(buf), L"%d", lineNum);
                lineRect.top = topOffset + i * lineHeight;
